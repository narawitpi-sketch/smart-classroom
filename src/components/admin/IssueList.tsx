  ...
  const handleDirectRepairSubmit = async (e: React.FormEvent) => {
    ...
    try {
       ...
       const docRef = await addDoc(collection(db, 'artifacts', APP_ID, 'public', 'data', 'issues'), {
          description: directProblem.trim(), // Note: Changed 'details' to 'description' to match existing schema if needed, but previously used 'details'. Let's check Issue type.
          room: directRoom.trim(),
          reporter: 'Admin (ซ่อมเอง)', // Changed 'reporterName' to 'reporter' to match existing schema
          reporterType: 'lecturer', // Default to lecturer or admin
          status: 'in-progress', // Note: 'in-progress' vs 'in_progress'. Check StatusBadge.
