...
// --- 3.2 Landing Screen ---
const LandingScreen = ({ onReporterClick, onAdminClick }: any) => (
...
const ReporterScreen = ({ rooms, formData, setFormData, onSubmit, onLogout, formSubmitting }: any) => {
  ...
  // Validation Handlers
  const handleRoomChange = (e: ChangeEvent<HTMLSelectElement>) => setFormData({ ...formData, room: e.target.value });
  const handleNameChange = (e: ChangeEvent<HTMLInputElement>) => { if (e.target.value === '' || /^[a-zA-Z\u0E00-\u0E7F\s]+$/.test(e.target.value)) setFormData({ ...formData, reporter: e.target.value }); };
  const handlePhoneChange = (e: ChangeEvent<HTMLInputElement>) => { if ((e.target.value === '' || /^[0-9]+$/.test(e.target.value)) && e.target.value.length <= 10) setFormData({ ...formData, phone: e.target.value }); };
...
export default function App() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/popup-closed-by-user') { setRole('guest'); }
      else { fireAlert('เกิดข้อผิดพลาด', error.message, 'error'); }
  ...
  const statsData = useMemo(() => {
    ...
    return { daily: fmt(stats.daily).slice(0, 7), monthly: fmt(stats.monthly), yearly: fmt(stats.yearly), byCategory: fmt(stats.byCategory), byReporter: fmt(stats.byReporter) };
  }, [issues]);
  ...
  return (
    <>
      ...
      {role === 'guest' && <LandingScreen onReporterClick={() => setRole('reporter')} onAdminClick={handleStaffClick} />}
